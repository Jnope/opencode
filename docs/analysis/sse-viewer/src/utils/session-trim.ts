/**
 * Session trim — ported from packages/app/src/context/global-sync/session-trim.ts
 * Limits the number of sessions kept in the store.
 */

import type { PermissionRequest, Session } from "./types"
import { cmp } from "./binary"

const SESSION_RECENT_WINDOW = 4 * 60 * 60 * 1000
const SESSION_RECENT_LIMIT = 50

function sessionUpdatedAt(session: Session): number {
  return session.time.updated ?? session.time.created
}

function compareSessionRecent(a: Session, b: Session): number {
...
function takeRecentSessions(sessions: Session[], limit: number, cutoff: number): Session[] {
  if (limit <= 0) return []
